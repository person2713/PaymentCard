...
function deleteRow() {
    ...
    $("table").find("tr").each(function () {
        if ($(this).find("input").is(":checked")) {
            console.log($(this).find("idUniq").html());
            mass.push($(this).find("id").html());
            $(this).remove();
        }
    });
    console.log(mass);
...
function createTableBodyForUsers(response, tdNick) {
    ...
    tBody += '<tbody>';
    $.each(response, function (i, item) {
        tBody += '<tr><td id="idUniq" style="display:none">' +
...
function saveChangesForUsers() {
    ...
    // переменная в которую записывает информация об объекте и которая передается на сервер
    var entity;
